...
it('can add the star name and star symbol properly', async() => {
    ...
    let user1 = accounts[1];
    let starId = 6;
    await instance.createStar('Polux', starId, {from: user1});
    ...
    assert.equal(starLookUp,'Polux');
});

it('lets 2 users exchange stars', async() => {
    let instance = await StarNotary.deployed();
    let Bernat = accounts[0];
    let Laia = accounts[1];
    let starId = 7;
    let starId2 = 9;
    // 1. create 2 Stars with different tokenId
    await instance.createStar('Gemini',starId, {from: Bernat});
    await instance.createStar('Unicorn',starId2, {from: Laia});
    ...
    assert.equal(star1,Laia);
    assert.equal(star2,Bernat);
});
...
it('lets a user transfer a star', async() => {
    ...
    let starId = 8;
    // 1. create a Star with different tokenId
    await instance.createStar('Polaris',starId, {from: Bernat});
    // 2. use the transferStar function implemented in the Smart Contract
    await instance.transferStar(Laia,starId);
    // 3. Verify the star owner changed.
    let stars = await instance.ownerOf.call(starId);
    assert.equal(stars,Laia);
});